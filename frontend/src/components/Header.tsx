        ...
        <nav className="flex items-center gap-1 md:gap-2">
          {navItems.map((item, index) => (
            <motion.button
              key={item.label}
              onClick={item.action}
              className={`relative px-3 md:px-6 py-2 md:py-2.5 rounded-full text-xs md:text-sm backdrop-blur-xl border transition-all duration-300 group overflow-hidden ${item.primary
